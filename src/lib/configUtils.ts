  ...
  try {
    ...
    if (lastChangeIndex > 0) {
      ...
      if (!robotPart || !robotPart.robotConfig) {
        console.error(
          'Invalid config structure in history entry (from history)'
        )
        return null
      }
      ...
      baseConfig = robotPartResponse.part.robotConfig
        ? robotPartResponse.part.robotConfig.toJson()
        : null
    }
      ...
      for (const fragmentMod of baseConfig.fragment_mods) {
        if (fragmentMod.mods && Array.isArray(fragmentMod.mods)) {
          fragmentMod.mods.sort((a: any, b: any) => {
            const keyA = a.$set
              ? Object.keys(a.$set)[0]
              : Object.keys(a.$unset)[0]
            const keyB = b.$set
              ? Object.keys(b.$set)[0]
              : Object.keys(b.$unset)[0]
...
export const getPassConfigComparison = (
  ...
  const currentPassIndex = allPasses.findIndex(
    (p) => p.pass_id === pass.pass_id
  )
  const prevPass =
    currentPassIndex > -1 && currentPassIndex < allPasses.length - 1
