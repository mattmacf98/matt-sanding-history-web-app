...
export class PassMetadataManager {
  ...
  private async getMetadata(): Promise<Record<string, JsonValue>> {
    if (this.cachedMetadata === null) {
      this.cachedMetadata = await this.viamClient.appClient.getRobotMetadata(
        this.machineId
      )
    }
  ...
  private async updateMetadata(
    ...
    await this.viamClient.appClient.updateRobotMetadata(
      this.machineId,
      currentMetadata
    )
  }
  ...
  private async getNotesMetadata(): Promise<Map<string, PassNote>> {
    ...
    this.cachedNotes = this.parseMetadataByPrefix<PassNote>(
      metadata,
      NOTE_PREFIX,
      'note'
    )
  ...
  private async getDiagnosesMetadata(): Promise<Map<string, PassDiagnosis>> {
    ...
    const metadata = await this.getMetadata()
    this.cachedDiagnoses = this.parseMetadataByPrefix<PassDiagnosis>(
      metadata,
      DIAGNOSIS_PREFIX,
      'diagnosis'
    )
    ...
    console.log(
      `🔍 Loaded ${this.cachedDiagnoses.size} diagnoses from metadata:`
    )
    if (this.cachedDiagnoses.size > 0) {
      console.table(
        Array.from(this.cachedDiagnoses.entries()).map(
          ([passId, diagnosis]) => ({
            passId,
  ...
  private async saveNotesMetadata(notes: Map<string, PassNote>): Promise<void> {
    ...
    if (notes.size > 0) {
      console.table(
        Array.from(notes.entries()).map(([passId, note]) => ({
          passId,
          noteText:
   ...
   */
  private async saveSingleDiagnosis(
    passId: string,
    diagnosis: PassDiagnosis
  ): Promise<void> {
    await this.updateMetadata((metadata) => {
  ...
  async savePassDiagnosis(
    ...
    console.log(
      `Saving diagnosis for pass ${passId}: symptom=${symptom}, cause=${cause}, jiraTicketUrl=${jiraTicketUrl}`
    )
 ...
 */
export function getPassMetadataManager(
